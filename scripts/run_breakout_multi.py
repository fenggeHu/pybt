...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run a Donchian breakout (stop orders) multi-symbol backtest")
    ...
    strategies = {sym: DonchianBreakout(symbol=sym, lookback=args.lookback, qty=args.qty) for sym in data.keys()}
    broker = SimBroker(
