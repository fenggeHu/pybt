...
def test_new_draft_config_contains_minimum_sections() -> None:
    ...
    assert cfg["data_feed"]["params"]["symbol"] == "000001.SZ"
    assert isinstance(cfg["strategies"], list)
    assert cfg["strategies"][0]["plugin"] == "moving_average"
    assert cfg["portfolio"]["plugin"] == "naive_portfolio"
    assert cfg["execution"]["plugin"] == "immediate_execution"


def test_parse_component_spec_with_json_payload() -> None:
    payload = _parse_component_spec(
        '{"plugin":"uptrend","symbol":"AAA","window":30}',
        require_type=True,
    )
    assert payload["plugin"] == "uptrend"
    assert payload["params"]["window"] == 30
...
def test_parse_component_spec_requires_type() -> None:
    try:
        ...
        assert "plugin" in str(exc).lower()
    else:
...
def test_build_effective_config_filters_disabled_strategies() -> None:
    ...
    cfg["strategies"] = [
        {"plugin": "moving_average", "params": {"symbol": "AAA"}, "enabled": True},
        {"plugin": "uptrend", "params": {"symbol": "AAA"}, "enabled": False},
    ]
    out = _build_effective_config(cfg)
    assert len(out["strategies"]) == 1
    assert out["strategies"][0]["plugin"] == "moving_average"
    assert "enabled" not in out["strategies"][0]


def test_match_strategy_indexes_supports_index_id_and_all() -> None:
    strategies = [
        {"plugin": "moving_average", "params": {"strategy_id": "mac-a"}},
        {"plugin": "uptrend", "params": {"strategy_id": "up-a"}},
    ]
